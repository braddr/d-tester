...
{
    Results r = mysql.query(text("select td.id, td.rc, td.test_run_id, td.test_type_id, tr.host_id, r.name from test_data td, test_runs tr, repositories r where r.id = td.repository_id and tr.id = td.test_run_id and td.id = ", testid));
    if (!r)
    {
